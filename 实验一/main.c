#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <windows.h>
#include <io.h>


#define MAXLEN 1024

//密钥对应的扩展数组
int w[44];
/**
 * S盒
 */
int S[16][16] = { 
};

/**
 * 逆S盒
 */
int S2[16][16] = { 
};


/**
 * 列混合要的矩阵
 */
int colM[4][4] = { 2, 3, 1, 1,
                   1, 2, 3, 1,
                   1, 1, 2, 3,
                   3, 1, 1, 2
};
/**
 * 逆列混合的矩阵
 */
int deColM[4][4] = { 0xe, 0xb, 0xd, 0x9,
                     0x9, 0xe, 0xb, 0xd,
                     0xd, 0x9, 0xe, 0xb,
                     0xb, 0xd, 0x9, 0xe
};

/**
 * 常量轮值表,密钥扩展用
 */
unsigned int Rcon[11] = { 0x00000000,
                          0x01000000, 0x02000000,
                          0x04000000, 0x08000000,
                          0x10000000, 0x20000000,
                          0x40000000, 0x80000000,
                          0x1b000000, 0x36000000
};

/**
 * 获取整型数据的低8位的左4个位
 */
int
getLeft4Bit(int num) { return (num&0x000000f0) >> 4; }

/**
 * 获取整型数据的低8位的右4个位
 */
int
getRight4Bit(int num) { return num & 0x0000000f; }

/**
 * 根据索引，从S盒中获得元素
 */
int
getNumFromSBox(int index) { return S[getLeft4Bit(index)][getRight4Bit(index)]; }

/**
 * 根据索引，从逆S盒中获得元素
 */
int
getNumFromS1Box(int index) { return S2[getLeft4Bit(index)][getRight4Bit(index)]; }

/** GF(2^8)有限域上的2*s
 * 多项式表示：x * f(x)
 * 参数 s: 多项式f(x)的二进制表示
 */
int GFMul2(int s)
{
    int result = s << 1;
    int a7 = result & 0x00000100;

    if(a7 != 0)
    {
        result = result & 0x000000ff;
        result = result ^ 0x1b;
    }

    return result;
}

/** GF(2^8)上的二元乘法运算
 * 参数 n: 第一操作数，不大于16
 * 参数 s: 第二操作数，只会取最低字节，毕竟GF(2^8)中只有8个系数
 */
int
GFMul(int n, int s)
{
    n &= 0x0f;
    s &= 0xff;
    int sum = s,
            result = 0;

    while (n!=0) {
        if (n & 1u) {
            result = (result ^ sum);
        }
        n = (n>>1);
        sum = GFMul2(sum);
    }
    return result;
}

/**
 * 把一个字符转变成整型
 */
int getIntFromChar(char c)
{
    return ((int)c) & 0x000000ff;
}

/**
 * 把16个字符转变成4X4的数组，
 * 该矩阵中字节的排列顺序为从上到下，从左到右依次排列。
 * pa = [
 *  [ str[0], str[4], str[8], str[12] ],
 *  [ str[1], str[5], str[9], str[13] ],
 *  [ str[2], str[6], str[10],str[14] ],
 *  [ str[3], str[7], str[11],str[15] ]
 * ]
 */
void convertToIntArray(char *str, int pa[4][4])
{
    int k = 0;
    int i,j;
    for(i = 0; i < 4; i++)
        for(j = 0; j < 4; j++)
        {
            pa[j][i] = getIntFromChar(str[k]);
            k++;
        }
}

/**
 * 把连续的4个字符合并成一个4字节的整型
 */
int getWordFromStr(char *str)
{
    int one, two, three, four;
    one = getIntFromChar(str[0]);
    one = one << 24;
    two = getIntFromChar(str[1]);
    two = two << 16;
    three = getIntFromChar(str[2]);
    three = three << 8;
    four = getIntFromChar(str[3]);
    return one | two | three | four;
}

/**
 * 把一个4字节的数的第一、二、三、四个字节取出，
 * 放入一个4个元素的整型数组里面。
 */
void splitIntToArray(int num, int array[4])
{
    int one, two, three;
    one = num >> 24;
    array[0] = one & 0x000000ff;
    two = num >> 16;
    array[1] = two & 0x000000ff;
    three = num >> 8;
    array[2] = three & 0x000000ff;
    array[3] = num & 0x000000ff;
}

/**
 * 把数组中的第一、二、三和四元素分别作为
 * 4字节整型的第一、二、三和四字节，合并成一个4字节整型
 */
int mergeArrayToInt(int array[4])
{
    int one = array[0] << 24;
    int two = array[1] << 16;
    int three = array[2] << 8;
    int four = array[3];
    return one | two | three | four;
}

/**
 * 字节替换
 */
void subBytes(int array[4][4])
{
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            array[i][j] = getNumFromSBox(array[i][j]);
        }
    }
}

/**
 * 逆字节替换
 */
void deSubBytes(int array[4][4])
{
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            array[i][j] = getNumFromS1Box(array[i][j]);
        }
    }
}

/**
 * 将数组中的元素循环左移step个
 */
void leftLoop4int(int array[4], int step)
{
    // 每次移动一位，移动step % 4 次
    for (int i = 0; i < step % 4; i++) {
        int x = array[0];
        for (int j = 0; j < 4; j++) {
            array[j] = array[j + 1];
        }
        array[3] = x;
    }
}

/**
 * 把4个元素的数组循环右移step个
 */
void rightLoop4int(int array[4], int step)
{
    // 每次移动一位，移动step % 4次
    for (int i = 0; i < step % 4; i++) {
        int x = array[3];
        for (int j = 3; j > 0; j--) {
            array[j] = array[j - 1];
        }
        array[0] = x;
    }
}

/**
 * 行移位
 */
void shiftRows(int array[4][4])
{
    // 按行移动，第i行循环左移i个字节
    for (int row_index = 0; row_index < 4; row_index++) {
        leftLoop4int(array[row_index], row_index);
    }
}

/**
 * 逆行移位
 */
void deShiftRows(int array[4][4])
{
    // 按行移动，第i行循环右移i个字节
    for (int row_index = 0; row_index < 4; row_index++) {
        rightLoop4int(array[row_index], row_index);
    }
}
/**
 * 列混合
 */
void mixColumns(int array[4][4])
{
    // 定义一个新的矩阵
    int answer[4][4];
    // 对于每一个元素而言
    for (int row_index = 0; row_index < 4; row_index++) {
        for (int col_index = 0; col_index < 4; col_index++) {
            // 计算矩阵中每一个元素的值
            answer[row_index][col_index] = 0;
            for (int k = 0; k < 4; k++) {
                // 在GF(2^8)域中的加法相当于异或
                answer[row_index][col_index] ^= GFMul(colM[row_index][k], array[k][col_index]);
            }
        }
    }
    // 新的矩阵覆盖原有的数据
    memcpy(array, answer, sizeof(answer));
}

/**
 * 逆列混合
 */
void deMixColumns(int array[4][4])
{
    // 定义一个新的矩阵
    int answer[4][4];
    // 对于每一个元素而言
    for (int row_index = 0; row_index < 4; row_index++) {
        for (int col_index = 0; col_index < 4; col_index++) {
            // 计算矩阵中每一个元素的值
            answer[row_index][col_index] = 0;
            // 解密乘以的矩阵不同
            for (int k = 0; k < 4; k++) {
                // 在GF(2^8)域中的加法相当于异或
                answer[row_index][col_index] ^= GFMul(deColM[row_index][k], array[k][col_index]);
            }
        }
    }
    // 新的矩阵覆盖原有的数据
    memcpy(array, answer, sizeof(answer));
}

/**
 * 轮密钥加
 */
void addRoundKey(int array[4][4], int round)
{
    int round_keys[4];
    // 取出当前轮数对应的轮密钥数组（矩阵的每一列对应一个轮密钥）
    for(int i = 0; i < 4; i++){
        round_keys[i] = w[round * 4 + i];
    }
    for(int row_index = 0; row_index < 4; row_index++) {
        int row_round_keys[4];
        // 将对应的轮密钥拆成一个矩阵的四列
        splitIntToArray(round_keys[row_index], row_round_keys);
        for (int col_index = 0; col_index < 4; col_index++) {
            // 因为读进来的时候是按照col_index, row_index 排序
            // 所以下标需要换一下
            array[col_index][row_index] ^= row_round_keys[col_index];
        }
    }
}

/**
 * 密钥扩展中的T函数（字循环，字节替换，异或）
 */
int T(int num, int round)
{
    int temp = (num << 8 | ((num >> 24) & 0xff));
    int tempArray[4];
    splitIntToArray(temp, tempArray);
    for (int i = 0; i < 4; i++) {
        tempArray[i] = getNumFromSBox(tempArray[i]);
    }
    temp = mergeArrayToInt(tempArray);
    // 循环左移一个字节
    return temp ^ Rcon[round];
}

/**
 * 扩展密钥，结果是把w[44]中的每个元素初始化
 */
void extendKey(char *key)
{
    char one[4], two[4], three[4], four[4];
    // 长度为16的字符串切割成4块
    strncpy(one, key, 4);
    strncpy(two, key+4, 4);
    strncpy(three, key+8, 4);
    strncpy(four, key+12, 4);

    // w数组的初始化,round = 0
    w[0] = getWordFromStr(one);
    w[1] = getWordFromStr(two);
    w[2] = getWordFromStr(three);
    w[3] = getWordFromStr(four);

    for (int round = 1; round < 12; round++) {
        for (int round_index = 0; round_index < 4; round_index++) {
            // w[i] = w[i-4] 异或 temp
            if (round_index == 0) {
                int temp = T(w[round * 4 + round_index - 1], round);
                w[round * 4 + round_index] = w[(round - 1) * 4 + round_index] ^ temp;
            }
            // w[i] = w[i-4] 异或 w[i-1]
            else {
                w[round * 4 + round_index] = w[(round - 1) * 4 + round_index] ^ w[round * 4 + round_index - 1];
            }
        }
    }
}
/**
 * 把4X4数组转回字符串
 */
void convertArrayToStr(int array[4][4], char *str)
{
    int i,j;
    for(i = 0; i < 4; i++)
        for(j = 0; j < 4; j++)
            *str++ = (char)array[j][i];
}
/**
 * 检查密钥长度
 */
int checkKeyLen(int len)
{
    if(len == 16)
        return 1;
    else
        return 0;
}

/**
 * 打印W数组-每一轮的密钥
 */
void printW()
{
    int i, j;
    for(i = 0, j = 1; i < 44; i++,j++)
    {
        printf("w[%d] = 0x%x ", i, w[i]);
        if(j % 4 == 0)
            printf("\n");
    }
    printf("\n");
}

/**
 *每一个分组的加密,必须为16的倍数
 *参数 p: 明文的字符串数组。
 * 参数 plen: 明文的长度。
 * 参数 key: 密钥的字符串数组。
 */
void aes(char *p, int plen, char *key)
{
    int keylen = strlen(key);
    int pArray[4][4];
    int k,i;

    if(plen == 0 || plen % 16 != 0)
    {
        printf("明文字符长度必须为16的倍数！\n");
        exit(0);
    }

    if(!checkKeyLen(keylen))
    {
        printf("密钥字符长度错误！长度必须为16。当前长度为%d\n",keylen);
        exit(0);
    }

    extendKey(key);//扩展密钥
    printW();
    for(k = 0; k < plen; k += 16)
    {
        convertToIntArray(p + k, pArray);
        // 正确
        addRoundKey(pArray, 0);//一开始的轮密钥加

        for(i = 1; i < 10; i++)
        {
            // 正确
            subBytes(pArray);//字节替换
            // 正确
            shiftRows(pArray);//行移位
            // 正确
            mixColumns(pArray);//列混合

            addRoundKey(pArray, i);

        }

        subBytes(pArray);//字节替换

        shiftRows(pArray);//行移位

        addRoundKey(pArray, 10);

        convertArrayToStr(pArray, p + k);
    }
}

/**
 * 参数 c: 密文的字符串数组。
 * 参数 clen: 密文的长度。
 * 参数 key: 密钥的字符串数组。
 */
void deAes(char *c, int clen, char *key)
{

    int cArray[4][4];
    int keylen,k, i;

    keylen = strlen(key);
    if(clen == 0 || clen % 16 != 0)
    {
        printf("密文字符长度必须为16的倍数！现在的长度为%d\n",clen);
        exit(0);
    }

    if(!checkKeyLen(keylen))
    {
        printf("密钥字符长度错误！长度必须为16、24和32。当前长度为%d\n",keylen);
        exit(0);
    }

    //请补充代码
    extendKey(key);     // 密钥拓展
    printW();

    for(k = 0; k < clen; k += 16)
    {
        convertToIntArray(c + k, cArray);
        // 正确
        addRoundKey(cArray, 10);//一开始的轮密钥加

        for(i = 1; i < 10; i++)
        {
            // 正确
            deShiftRows(cArray);    // 逆行移位

            // 正确
            deSubBytes(cArray);     // 逆字节替换

            addRoundKey(cArray, 10 - i);

            // 正确
            deMixColumns(cArray);   // 逆列混合

        }

        deShiftRows(cArray);        // 逆行移位

        deSubBytes(cArray);         // 逆字节替换

        addRoundKey(cArray, 0);

        convertArrayToStr(cArray, c + k);
    }
}

/**
*读取字符串
*/
void getString(char *str, int len)
{

    int slen = read(0, str, len);
    for(int i = 0; i < slen; i++,str++)
    {
        if(*str == '\n')
        {
            *str = '\0';
            break;
        }
    }
}
/**
*打印ASCCI码
*/
void printASCCI(char *str, int len)
{
    int c;
    for(int i = 0; i < len; i++)
    {
        c = (int)*str++;
        c = c & 0x000000ff;
        printf("0x%x ", c);
    }
    printf("\n");
}

/**
 * 从标准输入中读取用户输入的字符串
 */
void readPlainText(char *str, int *len)
{
    int plen;
    while(1)
    {
        getString(str, MAXLEN);
        plen = strlen(str);
        if(plen != 0 && plen % 16 == 0)
        {
            printf("你输入的明文为：%s\n", str);
            break;
        }
        else
        {
            printf("明文字符长度必须为16的倍数,现在的长度为%d\n", plen);
        }
    }
    *len = plen;
}
/**
 * 把字符串写进文件
 */
void writeStrToFile(char *str, int len, char *fileName)
{
    FILE *fp;
    fp = fopen(fileName, "wb");
    for(int i = 0; i < len; i++)
        putc(str[i], fp);
    fclose(fp);
}

/**
*加密后的密文字符写入文件
*/
void aesStrToFile(char *key)
{

    char p[MAXLEN];
    int plen;

    printf("请输入你的明文，明文字符长度必须为16的倍数\n");
    readPlainText(p,&plen);

    printf("轮密钥..................\n");
    aes(p, plen, key);//AES加密
    printf("进行AES加密..................\n");
    printf("加密完后的密文的ASCCI为：\n");
    printASCCI(p, plen);
    char fileName[64];
    printf("请输入你想要写进的文件名，比如'test.txt':\n");
    if(scanf("%s", fileName) == 1)
    {
        writeStrToFile(p, plen, fileName);
        printf("已经将密文写进%s中了,可以在运行该程序的当前目录中找到它。\n", fileName);
    }
}
/**
 * 从文件中读取字符串
 */
int readStrFromFile(char *fileName, char *str)
{
    FILE *fp = fopen(fileName, "rb");
    if(fp == NULL)
    {
        printf("打开文件出错，请确认文件存在当前目录下！\n");
        exit(0);
    }

    int i;
    for(i = 0; i < MAXLEN && (str[i] = getc(fp)) != EOF; i++);

    if(i >= MAXLEN)
    {
        printf("解密文件过大！\n");
        exit(0);
    }

    str[i] = '\0';
    fclose(fp);
    return i;
}


void deAesFile(char *key)
{
    char fileName[64];
    char c[MAXLEN];//密文字符串
    printf("请输入要解密的文件名，该文件必须和本程序在同一个目录\n");
    if(scanf("%s", fileName) == 1)
    {
        int clen = readStrFromFile(fileName, c);
        printf("开始解密.........\n");
        deAes(c, clen, key);
        printf("解密后的明文ASCII为：\n");
        printASCCI(c, clen);
        printf("明文为：%s\n", c);
        writeStrToFile(c,clen,fileName);
        printf("现在可以打开%s来查看解密后的密文了！\n",fileName);
    }
}

int main(int argc, char const *argv[])
{
    setbuf(stdout, NULL);
    char key[17];
    int klen;

    int cos=0;
    printf("************************$声明信息$****************************\n");
    printf("版权声明：未经授权，禁止传播、使用和用于商业用途\n");
    printf("使用说明：本程序是AES密码演示程序。\n");
    printf("**********************$声明信息$******************************\n");
    printf("================AES密码算法程序演示================\n\n");

    while(1)
    {
        printf("请输入16个字符的密钥：\n");
        getString(key,17);
        klen = strlen(key);
        if(klen != 16)
        {
            printf("请输入16个字符的密钥,当前密钥的长度为%d\n",klen);
        }
        else
        {
            printf("你输入的密钥为：%s\n",key);
            break;

        }

    }

    aesStrToFile(key);
    printf("是否开始解密,1解密，2退出\n");
    scanf("%d",&cos);
    if (cos == 1 )
    {
        deAesFile(key);
    }
    else
    {
        return 0;
    }
    system("pause");

    return 0;
}